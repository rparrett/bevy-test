...
#[derive(Default)]
struct Handles {
    font_one: Handle<Font>,
    font_two: Handle<Font>,
}

struct NotChangeyText;
struct ChangeyText;

// below a certain font size, the panic no longer happens and we end up making a complete
// loop around the string.
//
// on my system (macos, high dpi), the first crashy value seems to be 35.

static FONT_SIZE: f32 = 64.;

// it is not any particular glyph. the last glyph rendered changes depending on the font size.
...
fn change_text(
    mut query: Query<&mut Text, With<ChangeyText>>,
...
fn setup(commands: &mut Commands, asset_server: ResMut<AssetServer>, mut handles: ResMut<Handles>) {
    commands.spawn(OrthographicCameraBundle::new_2d());
                
    handles.font_one = asset_server.load("fonts/ComicNeue-Regular.ttf");
    handles.font_two = asset_server.load("fonts/NotoSansJP-Light.otf");

    // curiously, if I "pre-prepare" the font atlas by initializing the text with the
    // entire string here, we still end up panicing.

    // this can also be a TextBundle and the result is the same.
    
    commands.spawn(Text2dBundle {
        text: Text::with_section(
            "Hi".to_string(),
            TextStyle {
                font: handles.font_one.clone(),
                font_size: FONT_SIZE,
                color: Color::WHITE,
            },
            TextAlignment::default(),
        ),
        transform: Transform::from_xyz(0.0, FONT_SIZE + 10.0, 0.0),
        ..Default::default()
    })
    .with(NotChangeyText);
